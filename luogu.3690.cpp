...
inline void rotate(int now){
	...
	tree[tmp_node].son[kind] = tree[now].son[kind ^ 1]; tree[now].son[kind ^ 1] = tree[now].fa;
	if(tree[now].son[kind ^ 1]) tree[ tree[now].son[kind ^ 1] ].fa = tmp_node;
...
inline void cut(int from,int to){
	makeroot(from);
	if( (findroot(to) == from) && (tree[to].fa = from) && (!tree[to].son[0]) ){
		tree[to].fa = tree[from].son[1] = 0;	
		pushup(from);
	}
}
// Splay End
