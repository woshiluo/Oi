...
void rotate( int cur ) {
	int tmp_fa = tree[cur].fa; bool kind = get_son(cur);
	if( tmp_fa == rt ) {
		rt = cur; tree[cur].fa = 0;
	}
	else {
		tree[ tree[tmp_fa].fa ].son[ get_son(tmp_fa) ] = cur; tree[cur].fa = tree[tmp_fa].fa;
	}
	tree[tmp_fa].son[kind] = tree[cur].son[ kind ^ 1 ]; tree[ tree[tmp_fa].son[kind] ].fa = tmp_fa;
	tree[cur].son[ kind ^ 1 ] = tmp_fa; tree[tmp_fa].fa = cur;
...
int main() {
	...
	for( int i = 2, x; i <= n; i ++ ) {
		scanf( "%d", &x );
		int tmp_pre = pre( rt, x ); splay( tmp_pre ); 
		int tmp_nxt = nxt( rt, x ); splay( tmp_nxt );
