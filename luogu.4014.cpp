...
int pre[N << 3], dis[N << 3], que[N << 3], cur[N << 3], flow[N << 3];
bool vis[N << 3];
inline bool spfa_Mi(){
	...
	while(head <= tail || head_dep > tail_dep){
		for(int i = ehead[ que[head] ]; i; i = e[i].next){
			if(e[i].flow > 0 && dis[ que[head] ] + e[i].val < dis[ e[i].to ]){
				...
				if(!vis[ e[i].to ]){
					vis[ e[i].to ] = true;
					tail++;
					if(tail > (n << 2) + 10 ) tail = 0,tail_dep++;
					que[ tail ] = e[i].to;
				}
			}
		}
		vis[ que[head] ] = false; 
		head++;
		if(head > (n << 2) + 10 ) head = 0,head_dep++;
	}
	return pre[t] != 0;
}
...
int main(){
#ifndef ONLINE_JUDGE 
	freopen("luogu.4014.in","r",stdin);
	freopen("luogu.4014.out","w",stdout);
#endif
	scanf("%d",&n);
	s = (n<<1) + 2;
	t = (n<<1) + 3;
