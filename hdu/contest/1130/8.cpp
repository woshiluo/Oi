...
int main()
{
    ...
    while(T --> 0){
        ...
        memset(F4.data,0,sizeof F4.data);
        MATRIX A(1),B;
        for(int t=0;t<n;t++){
            for(int x=1;x<=n;x++)
            for(int y=1;y<=n;y++)
            (F4.data[x][y]+=A.data[x][y])%=mod;
            memcpy(B.data,A.data,sizeof B.data);
            memset(A.data,0,sizeof A.data);
            for(int i=1;i<=1;i++)
            for(int j=1;j<=N;j++)
            for(int k=1;k<=N;k++)
                (A.data[i][j]+=B.data[i][k]*F1.data[k][j])%=mod;
            // A=A*F1;
        }
        //F1.data[i][i]++;
        memcpy(F1.data,F4.data,sizeof F2.data);
        Rp=R-R%n;
        Lp=L-L%n+1;
        if(Lp<L)Lp+=n;
        // cout<<L<<' '<<R<<' '<<Lp<<' '<<Rp<<'\n';
