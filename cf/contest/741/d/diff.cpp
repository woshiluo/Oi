...
int main(){
	...
	system( "g++ d.cpp -o mine.run -g -lm -O2 -std=c++17 -Wall -Wextra" );
	system( "g++ dalao.cpp -o dalao.run -g -lm -O2 -std=c++17 -Wall -Wextra" );
	while(1){
		n++;
		system("./gen.run >t.in");
		system("time ./mine.run <t.in >mine.out");
		system("./dalao.run <t.in >dalao.out");
		if( system( "diff dalao.out mine.out -Z" ) != 0 ){
