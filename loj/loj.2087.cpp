...
namespace sub2 {
	...
	void calc( int cur ) {
		...
		auto get_s = []( int left, int rig ) { return sum[rig] - sum[ left - 1 ]; };
		auto len = []( int left, int rig ) { return rig - left + 1; };

		for( int i = cur; i <= n; i ++ ) {
			for( int p = 0; p <= n; p ++ ) {
				chk_Max( f[i][p], f[ i - 1 ][p] );
				for( int j = cur - 1; j < i; j ++ ) {
					chk_Max( f[i][ p + 1 ], ( f[j][p] + get_s( j + 1, i ) ) / len( j, i ) );
				}
			}
		}
...
int main() {
#ifdef woshiluo
	freopen( "loj.2087.in", "r", stdin );
	freopen( "loj.2087.out", "w", stdout );
#endif
	...
	if( p1 == n + 1 ) {
		std::cout << ((Decimal)h[1]).to_string(p);
		return 0;
	}
	if( n - p1 + 1 <= k ) {
		sub1::calc(p1);
